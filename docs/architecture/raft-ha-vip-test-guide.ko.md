...
smoke script는 `reverseproxy cluster bootstrap|join` CLI로 Raft cluster와 VIP lifecycle 입력을 수행한다.
...
```bash
docker network inspect reverseproxy-raft-ha-vip-macvlan_raft-ha-vip-net
```

...
기본 `compose.yaml`은 로컬 `./configs`를 사용합니다. 호스트 포트포워딩 포트는 환경 변수로 바꿀 수 있습니다.

```bash
LB_PROXY_PORT=18080 LB_DASHBOARD_PORT=19090 docker compose up -d
```
...
docker run --rm \
  ...
  loadbalancer serve /app/configs/app.json
```
